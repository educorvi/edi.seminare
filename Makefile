...
clean: ## Clean installation and instance (data left intact)
	...
	@rm -rf pyvenv.cfg .installed.cfg instance/etc .venv .pytest_cache .ruff_cache constraints* requirements*
...
i18n: $(VENV_FOLDER) ## Update locales
	...
	@$(BIN_FOLDER)/python -m edi.seminare.locales
...
test-coverage: $(VENV_FOLDER) ## run tests with coverage
	@$(BIN_FOLDER)/pytest --cov=edi.seminare --cov-report term-missing
